...
release() {
	...
	execute_cmd mvn -Darguments='-Dmaven.test.skip=true' release:prepare --batch-mode -Drelease -DdevelopmentVersion=$DEVELOPMENT_VERSION -DreleaseVersion=$RELEASE_VERSION -Dtag=vRELEASE_VERSION -DdryRun -DignoreSnapshots=true -Prelease
	if check_build_result; then     
	...
	if [ "$FLAG_PERFORM_RELEASE" == "y" ]; then
		...
		cd ../../
		execute_cmd mvn -Darguments='-Dmaven.test.skip=true' release:prepare --batch-mode -Drelease -DdevelopmentVersion=$DEVELOPMENT_VERSION -DreleaseVersion=$RELEASE_VERSION -Dtag=v$RELEASE_VERSION -Dresume=false -Prelease -DignoreSnapshots=true
		if check_build_result; then
			echo "Done."
		else
