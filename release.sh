...
release() {
	...
	execute_cmd mvn -Darguments="-Drelease -Prelease -DskipTests=true clean install" release:prepare --batch-mode
	...
	if [ "$FLAG_PERFORM_RELEASE" == "y" ]; then
		...
		cd ../../
		execute_cmd mvn -Darguments="-Drelease -Prelease clean install" release:prepare --batch-mode
