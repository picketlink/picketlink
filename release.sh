...
release() {
	...
	execute_cmd mvn release:prepare --batch-mode -Darguments="-Drelease -Prelease -DskipTests=true clean install"
	...
	if [ "$FLAG_PERFORM_RELEASE" == "y" ]; then
		...
		cd ../../
		execute_cmd mvn release:prepare --batch-mode -Darguments="-Drelease -Prelease clean install"
