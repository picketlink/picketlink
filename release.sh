...
release() {
	...
	execute_cmd mvn -DpreparationGoals="-Drelease -Prelease -DskipTests=true clean install" release:prepare
	--batch-mode -DdevelopmentVersion=$DEVELOPMENT_VERSION -DreleaseVersion=$RELEASE_VERSION -Dtag=vRELEASE_VERSION
	-DdryRun -DignoreSnapshots=true -Prelease
	if check_build_result; then     
	...
	if [ "$FLAG_PERFORM_RELEASE" == "y" ]; then
		...
		-DdevelopmentVersion=$DEVELOPMENT_VERSION -DreleaseVersion=$RELEASE_VERSION -Dtag=v$RELEASE_VERSION
		-Dresume=false -DignoreSnapshots=true -Prelease
		if check_build_result; then
			echo "Done."
		else
