...
public class FilePlainTextPasswordCredentialHandler implements CredentialHandler {
    ...
    public void update(Agent agent, Object credential, IdentityStore<?> store) {
        ...
        doUpdate(agent, (FileBasedIdentityStore) store, password);
    }

    protected void doUpdate(Agent agent, FileBasedIdentityStore store, PlainTextPassword password) {
        if (password.isEncodePassword()) {
            SHASaltedPasswordEncoder encoder = new SHASaltedPasswordEncoder(512);
