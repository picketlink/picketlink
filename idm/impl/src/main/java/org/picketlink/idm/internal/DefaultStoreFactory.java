...
public class DefaultStoreFactory implements StoreFactory {
    ...
    @Override
    public PartitionStore createPartitionStore(PartitionStoreConfiguration config, IdentityStoreInvocationContext context) {
        for (Class<? extends PartitionStoreConfiguration> cc : partitionConfigMap.keySet()) {
            if (cc.isInstance(config)) {
                try {
                    PartitionStore<PartitionStoreConfiguration> store = (PartitionStore<PartitionStoreConfiguration>) partitionConfigMap
                            .get(cc).newInstance();
                    store.setup(config, context);
                    return store;
                } catch (InstantiationException e) {
                    throw new SecurityConfigurationException("Exception while creating new PartitionStore instance", e);
                } catch (IllegalAccessException e) {
                    throw new SecurityConfigurationException("Exception while creating new PartitionStore instance", e);
                }
            }
        ...
        throw new IllegalArgumentException(
                "The PartitionStoreConfiguration specified is not supported by this IdentityStoreFactory implementation");
    }
