...
/**
 * <p>
 * A {@code ClaimsProcessor} implementation is responsible for parsing the WS-Trust claims according to the specified claims
 * dialect and retrieving the attributes that correspond to the required claims. {@code ClaimsProcessor}s may use the properties
 * specified in the configuration to perform its job (for instance, to connect to an external LDAP server or IDM system when
...
public interface ClaimsProcessor {
    ...
    /**
     * <p>
     * Parses the specified claims according to the claims dialect and returns a {@code Map} of attributes that correspond to
     * the required claims. Implementing classes may get the attributes from a local context or from an external system (like an
