 ...
 * @author Pedro Igor
 */
public class SAML2AssertionAndResponseSignatureTestCase {
    ...
    public void onSetup() throws Exception {
        ...
        handlerConfig.addParameter(SAML2SignatureGenerationHandler.SIGN_RESPONSE_AND_ASSERTION, true);

        Map<String, Object> chainOptions = new HashMap<String, Object>();
        IDPType idpType = new IDPType();
        chainOptions.put(GeneralConstants.CONFIGURATION, idpType);
        chainOptions.put(GeneralConstants.KEYPAIR, this.keypair);
    ...
    public void testSignatureWithHttpPostBinding() throws Exception {
        ...
        DefaultSAML2HandlerRequest handlerGenerationRequest = createHandlerRequest("POST", assertingPartyUrl, responseType);
        DefaultSAML2HandlerResponse handlerGenerationResponse = createHandlerResponse("POST", responseType);

        this.generationHandler.handleRequestType(handlerGenerationRequest, handlerGenerationResponse);

        Document resultingDocument = handlerGenerationResponse.getResultingDocument();
        ...
        assertEquals(2, signatureElement.getLength());

        assertEquals(resultingDocument.getDocumentElement(), signatureElement.item(0).getParentNode());

        Element signedAssertion = DocumentUtil.getChildElement(resultingDocument.getDocumentElement(), new QName(JBossSAMLURIConstants.ASSERTION_NSURI.get(), JBossSAMLConstants.ASSERTION.get()));

        assertEquals(signedAssertion, signatureElement.item(1).getParentNode());
