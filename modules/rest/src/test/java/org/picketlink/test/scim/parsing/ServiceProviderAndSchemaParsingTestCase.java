...
import org.picketlink.scim.model.v11.SCIMSchemaElement;
import org.picketlink.scim.model.v11.ServiceProviderConfiguration;
import org.picketlink.scim.model.v11.SCIMSchemaElement.Attributes;
import org.picketlink.scim.model.v11.SCIMSchemaElement.SubAttributes;
...
public class ServiceProviderAndSchemaParsingTestCase {
    ...
    public void parseSchema() throws Exception {
        ...
        SCIMParser parser = new SCIMParser();
        SCIMSchema sp = parser.parseSchema(is);
        assertNotNull(sp);

        SCIMSchemaElement[] schemaElements = sp.getSchemaElements();
        assertEquals(3, schemaElements.length);
        assertEquals(schemaElements[0].getId(), "urn:ietf:params:scim:schemas:core:2.0:User");
        assertEquals(schemaElements[0].getName(), "User");
        assertEquals(schemaElements[0].getDescription(), "User Account");

        Attributes attributes[] = schemaElements[0].getAttributes();
        ...
        Meta meta = schemaElements[0].getMeta();
