 ...
 */
@HttpSecurity
public enum HttpSecurityCorsPathConfig {

    @Path(pathGroup = "", pathName = "/corsProtectedUri/*")
    @Cors
    @AllowedOrigins(origins = { "http://www.example.org:9000", "http://www.example.com:8008" })
    @SupportedMethods(methods = { "GET", "PUT", "POST", "DELETE", "OPTIONS" })
    @SupportedHeaders (headers = { "Origin", "X-Requested-With", "Content-Type", "Accept", "Authorization" })
    @ExposedHeaders(headers = { "X-Requested-With", "Content-Type" })
    @SupportsCredentials
    @MaxAge(age = 3600)
    @Authc
    ...
    @PathGroup(pathGroupName = "REST Service Group")
    @Cors
    @AllowedOrigins(origins = { "http://www.example.org:9000", "http://www.example.com:8008" })
    @SupportedMethods(methods = { "POST", "DELETE", "OPTIONS" })
    @SupportedHeaders (headers = { "Origin", "X-Requested-With", "Content-Type", "Accept", "Authorization" })
    @ExposedHeaders(headers = { "Authorization", "Content-Type" })
    @SupportsCredentials
    @MaxAge(age = 3600)
    @Authc
    @Form(errorPage = "/errorA.html", loginPage = "/loginA.html", restoreOriginalRequest = "no")
    ...
    @AllowedRoles(roles = { "Role A", "Role B" })
    @Expressions(expressions = { "#{identity.isLoggedIn()}" })
    @Path(pathGroup = "REST Service Group", pathName = "/rest/b/*")
    OWNER_CORS,
