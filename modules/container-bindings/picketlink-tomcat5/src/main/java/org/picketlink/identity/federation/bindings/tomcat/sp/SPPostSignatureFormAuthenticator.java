...
public class SPPostSignatureFormAuthenticator extends SPPostFormAuthenticator {
         ...
         boolean willSendRequest) throws ProcessingException, ConfigurationException, IOException
   {
      ...
      Node nextSibling = getNextSibling(samlDocument);
      if(nextSibling != null)
      {
   ...
   private Node getNextSibling(Document samlDocument)
   {
       Element elem = samlDocument.getElementById("NameID");
       if(elem == null)
       {
           elem = samlDocument.getElementById("NameIDPolicy");
       }
       return elem;
   }
}